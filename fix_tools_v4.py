...
# Define the new showGeminiLiveWindow function
new_func = r"""function showGeminiLiveWindow()
    ...
    if geminiLiveGoogleSearchEnabled then
        toolsPart = "tools: [{ google_search: {} }],"
    end

    local sysInstr = geminiLiveSystemInstruction or "أنت مساعد صوتي ذكي. مهمتك الرد المباشر بصوتك فقط."
    if geminiLiveGoogleSearchEnabled then
        sysInstr = sysInstr .. " لديك القدرة على استخدام بحث جوجل للحصول على معلومات دقيقة ومحدثة، استخدمه دائماً عند سؤالك عن معلومات حالية."
    end
...
# Escape backslashes for python regex
