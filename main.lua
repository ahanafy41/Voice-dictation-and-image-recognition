...
local currentAppVersion = 1.81
local versionUrl = "https://raw.githubusercontent.com/ahanafy41/Voice-dictation-and-image-recognition/main/version.txt"
local updateUrl = 'https://raw.githubusercontent.com/ahanafy41/Voice-dictation-and-image-recognition/main/'
...
function showResultWindow(titleTextStr, contentTextStr)
    ...
    local scrollV = ScrollView(service)
    scrollV.setFillViewport(true) -- Ensure child expands to fill scrollview
    globalResultContentTextView = TextView(service)
    globalResultContentTextView.setText(contentTextStr)
    globalResultContentTextView.setTextIsSelectable(true)
    globalResultContentTextView.setTextSize(18)
    globalResultContentTextView.setTextColor(0xFFE0E0E0)
    globalResultContentTextView.setPadding(15, 25, 15, 25)
    scrollV.addView(globalResultContentTextView)

    local scrollParams = LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, 0, 1.0)
    scrollParams.setMargins(0, 10, 0, 10)
    resultWindow.addView(scrollV, scrollParams)
    ...
    local winP = WindowManager.LayoutParams()
    winP.width = WindowManager.LayoutParams.MATCH_PARENT
    -- Determine max height (80% of screen)
    local dm = service.getResources().getDisplayMetrics()
    winP.height = WindowManager.LayoutParams.WRAP_CONTENT
    -- Added max height constraint implicitly via layout params of the scrollview (already 1.0 weight)
    -- But let's set a fixed max height for the window itself if content is too long
    if #contentTextStr > 1000 then
        winP.height = math.floor(dm.heightPixels * 0.8)
    end
    winP.type = WindowManager.LayoutParams.TYPE_ACCESSIBILITY_OVERLAY
    winP.flags = WindowManager.LayoutParams.FLAG_NOT_TOUCH_MODAL | WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN
    winP.format = PixelFormat.TRANSLUCENT
    winP.gravity = Gravity.CENTER
    winP.horizontalMargin = 0.05
    winP.verticalMargin = 0.05
    pcall(function() wm.addView(resultWindow, winP) end)
end
...
-- ### Settings Management
-- ### Settings Management (Modularized) ###
function loadSettingsModule()
    if _G.settingsModuleLoaded then return true end
    local scriptPath = debug.getinfo(1, 'S').source:sub(2)
    local dir = scriptPath:match('(.*/)') or ''
    local targetPath = dir .. 'settings_module.lua'

    local function tryDofile()
        local success, err = pcall(function()
            local f = io.open(targetPath, 'r')
            if f then f:close() dofile(targetPath) else error('File not found') end
        end)
        return success, err
    end

    local success, err = tryDofile()

    if success then
        _G.settingsModuleLoaded = true
        return true
    else
        -- Emergency download
        service.asyncSpeak("جاري تحميل ملف الإعدادات المفقود، يرجى المحاولة مرة أخرى بعد ثوانٍ...")
        local fileUrl = updateUrl .. "settings_module.lua"
        Http.get(fileUrl, nil, "UTF-8", nil, function(code, body)
            if code == 200 and body and #body > 100 then
                pcall(function()
                    ...
                    osw.close()
                    _G.settingsModuleLoaded = true
                    service.asyncSpeak("اكتمل التحميل الطارئ. يمكنك فتح الإعدادات الآن.")
                end)
            end
        end)
        print('Failed to load settings_module.lua locally: ' .. tostring(err))
        return false
    end
end

function saveSettings(...) if loadSettingsModule() then return _G.saveSettings(...) end end
function hideSettings(...) if loadSettingsModule() then return _G.hideSettings(...) end end
function openAiSettingsWindow(...) if loadSettingsModule() then return _G.openAiSettingsWindow(...) end end
function openSettings(...) if loadSettingsModule() then return _G.openSettings(...) end end
...
function downloadUpdateAndApply()
    service.asyncSpeak("جاري تنزيل التحديثات...")
    showResultWindow("تحديث الإضافة", "⏳ جاري تنزيل ملفات التحديث من الخادم...")

    local filesToUpdate = {"main.lua", "settings_module.lua"}
    ...
    for _, fileName in ipairs(filesToUpdate) do
        local fileUrl = updateUrl .. fileName
        Http.get(fileUrl, nil, "UTF-8", nil, function(code, body)
            if code == 200 and body and #body > 100 then
                local success, err = pcall(function()
                    import "java.io.File"
                    import "java.io.FileOutputStream"
                    import "java.io.OutputStreamWriter"
                    local file = File(scriptDir .. fileName)
                    local fos = FileOutputStream(file)
                    local osw = OutputStreamWriter(fos, "UTF-8")
                    osw.write(body)
                    osw.flush()
                ...
                hasError = true
                errorDetail = errorDetail .. "\n" .. fileName .. ": Failed to download (Code " .. code .. ")"
            end

            downloadedCount = downloadedCount + 1
            if downloadedCount == #filesToUpdate then
                mainHandler.post(luajava.createProxy("java.lang.Runnable", {
                    run = function()
                        if not hasError then
                            service.asyncSpeak("تم تحديث كافة الملفات بنجاح! يرجى إغلاق الإضافة وفتحها مرة أخرى.")
                            showResultWindow("تم التحديث ✅", "تم تثبيت التحديث الجديد بنجاح لجميع الوحدات.\n\nيرجى الرجوع وإغلاق الإضافة، ثم فتحها من جديد ليعمل الإصدار الجديد.")
                        else
                            service.asyncSpeak("حدث خطأ أثناء بعض التحديثات.")
                            showResultWindow("خطأ ❌", "فشل في تحديث بعض الملفات: " .. errorDetail)
                        end
                    end
                }))
            end
