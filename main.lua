...
function loadSettingsModule()
    ...
    local success, err = pcall(function()
        local f = io.open(dir .. 'settings_module.lua', 'r')
        if f then f:close() dofile(dir .. 'settings_module.lua') else error('File not found') end
    end)
    if success then
        _G.settingsModuleLoaded = true
        return true
    else
        print('Failed to load settings_module.lua: ' .. tostring(err))
        service.asyncSpeak('خطأ في تحميل وحدة الإعدادات.')
        return false
    end
end
