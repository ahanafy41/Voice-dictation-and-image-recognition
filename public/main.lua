...
-- **Google Gemini Models (Official 2026 - 100% Google Native)**
local geminiModels = {
    { name = "Gemini 3.1 Flash-Lite (الأسرع والأحدث - موصى به)", id = "gemini-3.1-flash-lite" },
    { name = "Gemini 3 Flash (أداء متوازن فائق)", id = "gemini-3-flash" },
    { name = "Gemini 2.5 Flash (مستقر وسريع)", id = "gemini-2.5-flash" },
    { name = "Gemini 2.5 Pro (أعلى ذكاء للمستندات والتحليل)", id = "gemini-2.5-pro" }
}
...
-- **Audio Models (100% Google Gemini Multimodal)**
local audioModels = {
    { name = "Gemini 3.1 Flash-Lite (الأسرع والأدق في التفريغ)", id = "gemini-3.1-flash-lite", provider = "gemini" },
    { name = "Gemini 3 Flash (تفريغ ذكي فائق الدقة)", id = "gemini-3-flash", provider = "gemini" },
    { name = "Gemini 2.5 Flash (مستقر وسريع)", id = "gemini-2.5-flash", provider = "gemini" },
    { name = "Gemini 2.5 Pro (أعلى دقة للملفات الصوتية الطويلة)", id = "gemini-2.5-pro", provider = "gemini" }
}
local defaultAudioModelId = "gemini-3.1-flash-lite"
...
-- Load cached Gemini models from SharedPreferences if available
local cachedGeminiModelsJson = prefs.getString("cachedGeminiModels", "")
if cachedGeminiModelsJson ~= "" then
    local success, jArr = pcall(function() return JSONArray(cachedGeminiModelsJson) end)
    if success and jArr and jArr.length() > 0 then
        local loadedModels = {}
        for i = 0, jArr.length() - 1 do
            local item = jArr.getJSONObject(i)
            table.insert(loadedModels, {
                name = item.getString("name"),
                id = item.getString("id")
            })
        end
        if #loadedModels > 0 then geminiModels = loadedModels end
    end
end
...
local loadedAudioModelId = prefs.getString("audioModelId", defaultAudioModelId)
selectedAudioModelId = loadedAudioModelId or defaultAudioModelId
...
-- ### UNIFIED AI REQUEST FUNCTION (100% Google Gemini Official Engine) ###
function makeAiRequest(prompt, systemInstruction, imageBase64, modelIdOverride, callback, tempOverride)
    ...
    local model = modelIdOverride or selectedGeminiModelId or defaultGeminiModelId
    if not apiKey or apiKey == "" then
        callback("Error: مفتاح Google Gemini API غير متوفر. يرجى إدخاله في الإعدادات.")
        return
    end
    ...
    Http.post(url, requestBody, headers, function(status, response)
            ...
            if success and jsonResponse then
                ...
                if jsonResponse.has("candidates") and jsonResponse.getJSONArray("candidates").length() > 0 then
                    ...
                    if cand.has("content") and cand.getJSONObject("content").has("parts") then
                         local resParts = cand.getJSONObject("content").getJSONArray("parts")
                         if resParts.length() > 0 and resParts.getJSONObject(0).has("text") then
                             resultText = resParts.getJSONObject(0).getString("text")
                         end
                    end
                end
        ...
        local errMsg = "Error " .. status .. ". Check your Google Gemini API key or model availability."
        if response and response:match("error") then
            pcall(function()
                local ej = JSONObject(response)
                if ej.has("error") then
                    local err = ej.get("error")
                    if err.getClass().getSimpleName() == "String" then
                        errMsg = tostring(err)
                    elseif ej.getJSONObject("error").has("message") then
                        errMsg = ej.getJSONObject("error").getString("message")
                    end
                end
            end)
        end
        callback("AI Request Failed: " .. errMsg .. " (Status: " .. status .. ")")
    end)
end
...
function correctWithAi(text, callback)
    ...
    local temp = 0.0
    local providerModelId = selectedGeminiModelId or defaultGeminiModelId
...
function transcribeWithGemini(filePath, callback, modelId)
    local apiKey = geminiApiKey
    if not apiKey or apiKey == "" then
        callback("Error: مفتاح Google Gemini API غير متوفر. يرجى إدخاله من الإعدادات.", true)
        return
    end

    local modId = modelId or selectedAudioModelId or "gemini-3.1-flash-lite"
    local ext = filePath:match("%.([^%.]+)$") or "mp3"
    local mime = "audio/" .. ext
    if ext == "mp3" then mime = "audio/mpeg" end
    if ext == "m4a" then mime = "audio/mp4" end
    if ext == "wav" then mime = "audio/wav" end
    ...
    uploadFileToGemini(filePath, mime, apiKey, function(fileUriOrError)
        ...
        local prompt = "قم بتفريغ هذا الملف الصوتي بدقة واحترافية. اكتب النص المستخرج فقط دون أي مقدمات أو شروحات."
        ...
        Http.post(url, root.toString(), headers, function(status, response)
            ...
            callback("Error Gemini Audio Transcription: " .. status .. " - " .. tostring(response), true)
        end)
    end)
end

function transcribeAudio(filePath, callback)
    local modId = selectedAudioModelId or defaultAudioModelId or "gemini-3.1-flash-lite"
    service.asyncSpeak("تفريغ الصوت عبر Google Gemini: " .. modId)
...
-- **Fetch Gemini Models Dynamically from Google**
function fetchGeminiModels(callback)
    if not geminiApiKey or geminiApiKey == "" then
        service.asyncSpeak("يرجى إدخال مفتاح Google Gemini API أولاً.")
        return
    end
    service.asyncSpeak("جاري جلب قائمة موديلات Google Gemini...")
    local url = "https://generativelanguage.googleapis.com/v1beta/models?key=" .. geminiApiKey

    Http.get(url, nil, "UTF-8", nil, function(code, body)
        if code == 200 then
            local s, j = pcall(function() return JSONObject(body) end)
            if s and j and j.has("models") then
                local modelsArr = j.getJSONArray("models")
                local newModels = {}
                for i = 0, modelsArr.length() - 1 do
                    local item = modelsArr.getJSONObject(i)
                    local rawName = item.getString("name") -- format: models/gemini-xxx
                    local cleanId = rawName:gsub("^models/", "")
                    if cleanId:match("^gemini") and not cleanId:match("embedding") and not cleanId:match("aqa") then
                        local displayName = item.has("displayName") and item.getString("displayName") or cleanId
                        table.insert(newModels, {name = displayName .. " (" .. cleanId .. ")", id = cleanId})
                    end
                end
                if #newModels > 0 then
                    geminiModels = newModels
                    local jArrToSave = JSONArray()
                    for _, m in ipairs(geminiModels) do
                        local jo = JSONObject()
                        jo.put("name", m.name)
                        jo.put("id", m.id)
                        jArrToSave.put(jo)
                    ...
                    local editor = prefs.edit()
                    editor.putString("cachedGeminiModels", jArrToSave.toString())
                    editor.apply()
                    service.asyncSpeak("تم تحديث القائمة: " .. #geminiModels .. " موديل من Google.")
                    if callback then callback() end
                else
                    service.asyncSpeak("لم يتم العثور على موديلات Gemini جديدة.")
                end
            else
                service.asyncSpeak("فشل تحليل بيانات Google Gemini.")
            end
        else
            service.asyncSpeak("فشل جلب موديلات Google Gemini. كود الخطأ: " .. code)
        end
    end)
end
-- ### UI Window Functions ###
...
-- ### Robust Input Helpers for Smart Agent (setText & paste) ###
function setText(text)
    if not text then return false end
    local str = tostring(text)

    -- 1. Try Jieshuo native service.setText (Standard Accessibility text replacement)
    local s1 = pcall(function()
        if service and service.setText then
            service.setText(str)
            return true
        end
        return false
    ...
    -- 2. Find currently focused editable node in active window and set text via ACTION_SET_TEXT
    local s2 = false
    pcall(function()
        local rootN = service.getRootInActiveWindow()
        if not rootN then return end

        local function findAndSetText(node)
            if not node then return false end
            local isEditable = false
            pcall(function() isEditable = node.isEditable() or node.isFocused() end)

            if isEditable then
                -- AccessibilityNodeInfo.ACTION_SET_TEXT = 2097152 (0x200000)
                local args = Bundle()
                args.putCharSequence("ACTION_ARGUMENT_SET_TEXT_CHARSEQUENCE", str)
                local actOk = false
                pcall(function() actOk = node.performAction(2097152, args) end)
                if not actOk then
                    pcall(function() actOk = node.performAction(16) end) -- click first
                end
                return actOk
            end

            local count = 0
            pcall(function() count = node.getChildCount() end)
            for i = 0, count - 1 do
                local child = nil
                pcall(function() child = node.getChild(i) end)
                if child then
                    if findAndSetText(child) then
                        pcall(function() child.recycle() end)
                        return true
                    end
                    pcall(function() child.recycle() end)
        ...
        s2 = findAndSetText(rootN)
        pcall(function() rootN.recycle() end)
    end)
    if s2 then return true end

    -- 3. Clipboard + Paste Fallback: Copy to clipboard and trigger paste
    local s3 = pcall(function()
        local cb = service.getSystemService(Context.CLIPBOARD_SERVICE)
        local clip = ClipData.newPlainText("AgentInput", str)
        cb.setPrimaryClip(clip)
        if service and service.paste then
            service.paste()
        else
            -- Try ACTION_PASTE on focused element (32768)
            local rootN = service.getRootInActiveWindow()
            if rootN then
                local focusedNode = rootN.findFocus(1) -- FOCUS_INPUT
                if focusedNode then
                    pcall(function() focusedNode.performAction(32768) end)
                    pcall(function() focusedNode.recycle() end)
                end
                pcall(function() rootN.recycle() end)
            end
        end
...
function paste(text)
    if text then
        local cb = service.getSystemService(Context.CLIPBOARD_SERVICE)
        local clip = ClipData.newPlainText("AgentPaste", tostring(text))
        cb.setPrimaryClip(clip)
    end

    if service and service.paste then
        local s = pcall(function() service.paste() end)
        if s then return true end
    end

    -- Fallback to Accessibility ACTION_PASTE (32768)
    local s2 = false
    pcall(function()
        local rootN = service.getRootInActiveWindow()
        if rootN then
            local focusedNode = rootN.findFocus(1) -- FOCUS_INPUT
            if focusedNode then
                s2 = focusedNode.performAction(32768)
                pcall(function() focusedNode.recycle() end)
            end
            pcall(function() rootN.recycle() end)
        end
...
function saveSettings()
    ...
    -- Clean up legacy and removed provider keys
    editor.remove("groqApiKey")
    editor.remove("witApiKey")
    editor.remove("groqModelId")
    editor.remove("cachedGroqModels")
...
function openAiSettingsWindow()
    ...
    local sections = {
        ...
        function() -- 2. Dictation Features
            ...
            -- Text Processor Provider (100% Google Gemini)
            local providerLabel = TextView(service)
            providerLabel.setText("محرك الذكاء الاصطناعي للإملاء والمعالجة:")
            providerLabel.setTextColor(0xFFE0E0E0); providerLabel.setTextSize(16); providerLabel.setTypeface(nil, Typeface.BOLD); providerLabel.setPadding(0, 30, 0, 5)
            dictationCard.addView(providerLabel)

            local providerBadge = TextView(service)
            providerBadge.setText("✨ Google Gemini الرسمي (الاعتماد الحصري 100%)")
            providerBadge.setTextColor(0xFF64B5F6); providerBadge.setTextSize(14); providerBadge.setTypeface(nil, Typeface.BOLD); providerBadge.setPadding(0, 5, 0, 15)
            dictationCard.addView(providerBadge)
...
-- ### GENIUS SETTINGS ROUTER (Fix for Weak Devices) ###
-- Instead of one huge window, we use a lightweight menu to navigate subsets of settings.
function openSettings()
    ...
    settingsDialog.setBackgroundColor(0xFF000000)
    settingsDialog.setPadding(35, 35, 35, 35)

    local titleTxt = TextView(service)
    titleTxt.setText("بوابة الإعدادات المتقدمة ⚙️")
    titleTxt.setTextSize(24)
    titleTxt.setTypeface(nil, Typeface.BOLD)
    titleTxt.setTextColor(0xFFFFFFFF)
    titleTxt.setGravity(Gravity.CENTER)
    titleTxt.setPadding(0, 0, 0, 50)
    settingsDialog.addView(titleTxt)

    local menuButtons = {
        { name = "🧠 إعدادات الإملاء والذكاء", desc = "تخصيص وضع الإملاء، التشكيل، والرموز التعبيرية", func = openAiSettingsWindow },
        { name = "🔑 مفاتيح الربط والنماذج", desc = "إدخال مفاتيح API واختيار موديلات الذكاء الاصطناعي", func = openConnectionSettings },
        { name = "🛠️ المميزات والأدوات", desc = "إعدادات الترجمة، الوصف التلقائي، والبحث الذكي", func = openFeaturesSettings },
        { name = "📱 الواجهة والتحديثات", desc = "ترتيب الأزرار، الزر العائم، والتحقق من التحديثات", func = openInterfaceSettings }
    }

    for _, btnInfo in ipairs(menuButtons) do
        local btn = Button(service)
        btn.setText(btnInfo.name)
        btn.setContentDescription(btnInfo.desc)
        styleButton(btn, "primary")
        local lp = LinearLayout.LayoutParams(-1, -2)
        lp.topMargin = 25
        btn.setOnClickListener(function()
            -- Genius Step: Wipe everything before loading next small module
            if settingsDialog then pcall(function() wm.removeView(settingsDialog) end); settingsDialog = nil end
            collectgarbage("collect")
            mainHandler.postDelayed(luajava.createProxy("java.lang.Runnable", { run = function() btnInfo.func() end }), 300)
        end)
        settingsDialog.addView(btn, lp)
    end
    ...
    local lpB = LinearLayout.LayoutParams(-1, -2); lpB.topMargin = 50; settingsDialog.addView(backBtn, lpB)

    local p = WindowManager.LayoutParams(-1, -1, WindowManager.LayoutParams.TYPE_ACCESSIBILITY_OVERLAY, WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN, -3)
    p.gravity = Gravity.CENTER
    pcall(function() wm.addView(settingsDialog, p) end)
    service.asyncSpeak("تم فتح بوابة الإعدادات. اختر القسم المطلوب.")
end

-- 1. Connection Settings (API Keys & Models)
function openConnectionSettings()
    if settingsDialog then return end
    collectgarbage("collect")
    service.asyncSpeak("تحميل إعدادات الاتصال...")
    ...
    local sections = {
        function() -- Header
            local header = TextView(service)
            header.setText("إعدادات الاتصال والربط 🔑")
        ...
        function() -- API Keys Card
            local apiCard = createCard(contentL)
            local h = TextView(service); h.setText("مفاتيح الربط 🔑"); h.setTextSize(18); h.setTypeface(nil, Typeface.BOLD); h.setTextColor(0xFF64B5F6); h.setPadding(0, 10, 0, 25); apiCard.addView(h)
            ...
            mIn = createApiKeyRow("مفتاح Google Gemini API:", geminiApiKey, "https://aistudio.google.com/app/apikey")
            tIn = createApiKeyRow("مفتاح Tavily API (اختياري للبحث):", tavilyApiKey, "https://app.tavily.com/")
        end,
        function() -- Models Card
            local modelCard = createCard(contentL)
            local h2 = TextView(service); h2.setText("نماذج Google Gemini 🤖"); h2.setTextSize(18); h2.setTypeface(nil, Typeface.BOLD); h2.setTextColor(0xFF64B5F6); h2.setPadding(0, 10, 0, 25); modelCard.addView(h2)

            modelCard.addView(createLabel("موديل تفريغ الصوت (Gemini Multimodal):"))
            local audNames = ArrayList(); audIds = {}
            for _, m in ipairs(audioModels) do audNames.add(m.name); table.insert(audIds, m.id) end
            audSpinner = Spinner(service); audSpinner.setAdapter(ArrayAdapter(service, android.R.layout.simple_spinner_item, audNames))
            local currAudIdx = 0; for i, id in ipairs(audIds) do if id == selectedAudioModelId then currAudIdx = i-1 break end end
            audSpinner.setSelection(currAudIdx); modelCard.addView(audSpinner)

            modelCard.addView(createLabel("موديل Gemini الأساسي (الإملاء، الترجمة، الرؤية):"))
            local gemNames = ArrayList(); gemIds = {}
            ...
            local gemFetchBtn = Button(service); gemFetchBtn.setText("🔄 تحديث قائمة نماذج Gemini من Google"); styleButton(gemFetchBtn, "secondary")
            gemFetchBtn.setOnClickListener(function() fetchGeminiModels(function() hideSettings(); openSettings() end) end)
            modelCard.addView(gemFetchBtn)
        ...
        function() -- Actions
            ...
            saveBtn.setOnClickListener(function()
                geminiApiKey = mIn.getText().toString(); tavilyApiKey = tIn.getText().toString()
            ...
            btnL.addView(saveBtn)
            local backBtn = Button(service); backBtn.setText("⬅️ رجوع"); styleButton(backBtn, "danger"); backBtn.setOnClickListener(function() hideSettings(); openSettings() end); local lp = LinearLayout.LayoutParams(-1, -2); lp.topMargin = 20; btnL.addView(backBtn, lp)
            contentL.addView(btnL)
        end
    }
    ...
    local function addSectionsSequentially(index)
        ...
        if index % 2 == 0 then collectgarbage("collect") end
        pcall(sections[index])
        mainHandler.postDelayed(luajava.createProxy("java.lang.Runnable", { run = function() addSectionsSequentially(index + 1) end }), 600)
    end
    addSectionsSequentially(1)

    pcall(function() wm.addView(settingsDialog, WindowManager.LayoutParams(-1, -1, WindowManager.LayoutParams.TYPE_ACCESSIBILITY_OVERLAY, 0, -3)) end)
end

-- 2. Features Settings (Translation, Vision, Live)
function openFeaturesSettings()
    if settingsDialog then return end
    collectgarbage("collect")
    service.asyncSpeak("تحميل المميزات...")
    ...
    local sections = {
        function() -- Header
            local titleTxt = TextView(service)
            titleTxt.setText("المميزات والأدوات الذكية 🛠️")
            titleTxt.setTextSize(22)
            titleTxt.setTypeface(nil, Typeface.BOLD)
            titleTxt.setTextColor(0xFFFFFFFF)
            titleTxt.setGravity(Gravity.CENTER_HORIZONTAL)
        ...
        function() -- Features Switches
            ...
            for _, l in ipairs(supportedLanguages) do trNames.add(l.name); table.insert(trLangIds, l.code) end
            trLangSpinner = Spinner(service); trLangSpinner.setAdapter(ArrayAdapter(service, android.R.layout.simple_spinner_item, trNames))
            local currTrIdx = 0; for i, id in ipairs(trLangIds) do if id == translateToLanguage then currTrIdx = i-1 break end end
            trLangSpinner.setSelection(currTrIdx); featCard.addView(trLangSpinner)

            swSum = Switch(service); swSum.setChecked(summarizeEnabled); createSettingRow("تلخيص النصوص", swSum, featCard)
            swImg = Switch(service); swImg.setChecked(imageDescriptionEnabled); createSettingRow("وصف الصور", swImg, featCard)

            featCard.addView(createLabel("نطاق التقاط الصور:"))
            local smNames = ArrayList(); smIds = {"full", "focus"}
            smNames.add("كامل الشاشة"); smNames.add("العنصر المحدد")
            smSpinner = Spinner(service); smSpinner.setAdapter(ArrayAdapter(service, android.R.layout.simple_spinner_item, smNames))
            local currSmIdx = 0; if screenshotMode == "focus" then currSmIdx = 1 end
            smSpinner.setSelection(currSmIdx); featCard.addView(smSpinner)
        end,
        function() -- Gemini Live Card
            local liveCard = createCard(contentL)
            local h = TextView(service); h.setText("إعدادات Gemini Live 🎙️"); h.setTextSize(18); h.setTypeface(nil, Typeface.BOLD); h.setTextColor(0xFF64B5F6); h.setPadding(0, 10, 0, 25); liveCard.addView(h)

            liveCard.addView(createLabel("تعليمات البث (System Instruction):"))
            liveSysIn = EditText(service); liveSysIn.setText(geminiLiveSystemInstruction or ""); styleEditText(liveSysIn); liveCard.addView(liveSysIn)

            liveCard.addView(createLabel("صوت المساعد:"))
            local vNames = ArrayList(); liveVoiceIds = {}
            for _, v in ipairs(geminiLiveVoices) do vNames.add(v.name); table.insert(liveVoiceIds, v.id) end
            liveVoiceSpinner = Spinner(service); liveVoiceSpinner.setAdapter(ArrayAdapter(service, android.R.layout.simple_spinner_item, vNames))
            local currVIdx = 0; for i, id in ipairs(liveVoiceIds) do if id == geminiLiveVoiceName then currVIdx = i-1 break end end
            liveVoiceSpinner.setSelection(currVIdx); liveCard.addView(liveVoiceSpinner)
        end,
        function() -- Actions
            local btnL = LinearLayout(service); btnL.setOrientation(LinearLayout.VERTICAL); btnL.setGravity(Gravity.CENTER); btnL.setPadding(0, 40, 0, 10)
            local saveBtn = Button(service); saveBtn.setText("💾 حفظ"); styleButton(saveBtn, "primary")
            saveBtn.setOnClickListener(function()
                ...
                screenshotMode = smIds[smSpinner.getSelectedItemPosition() + 1]
                geminiLiveSystemInstruction = liveSysIn.getText().toString()
                geminiLiveVoiceName = liveVoiceIds[liveVoiceSpinner.getSelectedItemPosition() + 1]
                saveSettings()
                if settingsDialog then pcall(function() wm.removeView(settingsDialog) end); settingsDialog = nil end
            end)
            btnL.addView(saveBtn)
...
-- 3. Interface Settings (Updates, Dashboard, Floating Button)
function openInterfaceSettings()
    if settingsDialog then return end
    collectgarbage("collect")
    service.asyncSpeak("تحميل إعدادات الواجهة...")

    settingsDialog = LinearLayout(service); settingsDialog.setOrientation(LinearLayout.VERTICAL); settingsDialog.setBackgroundColor(0xFF121212); settingsDialog.setPadding(35,35,35,35)
    local scrollV = ScrollView(service); local contentL = LinearLayout(service); contentL.setOrientation(LinearLayout.VERTICAL); scrollV.addView(contentL)
    settingsDialog.addView(scrollV)
    ...
    local sortBtn = Button(service); sortBtn.setText("🔢 ترتيب اللوحة"); styleButton(sortBtn, "secondary"); sortBtn.setOnClickListener(function()
        ...
        settingsDialog = LinearLayout(service); settingsDialog.setOrientation(LinearLayout.VERTICAL); settingsDialog.setBackgroundColor(0xFF121212); settingsDialog.setPadding(30, 30, 30, 30)

        local h = TextView(service); h.setText("ترتيب عناصر اللوحة 🔢"); h.setTextSize(20); h.setTypeface(nil, Typeface.BOLD); h.setTextColor(0xFF64B5F6); h.setGravity(Gravity.CENTER); h.setPadding(0, 0, 0, 30); settingsDialog.addView(h)
        ...
        local function refreshUI()
            ...
            local keys = {}
            for k in dashboardOrder:gmatch("([^,]+)") do local ck = k:gsub("^%s+", ""):gsub("%s+$", ""); if keyNames[ck] then table.insert(keys, ck) end end

            for i, k in ipairs(keys) do
                local row = createCard(listL)
                local content = LinearLayout(service); content.setOrientation(LinearLayout.HORIZONTAL); content.setGravity(Gravity.CENTER_VERTICAL); row.addView(content)

                local nameTv = TextView(service); nameTv.setText(keyNames[k]); nameTv.setTextColor(0xFFFFFFFF); nameTv.setTextSize(16); content.addView(nameTv, LinearLayout.LayoutParams(0, -2, 1.0))

                if i > 1 then
                    local up = Button(service); up.setText("🔼"); styleButton(up, "secondary"); up.setPadding(10, 10, 10, 10); up.setOnClickListener(function()
                        keys[i], keys[i-1] = keys[i-1], keys[i]; dashboardOrder = table.concat(keys, ","); refreshUI(); service.asyncSpeak("تم النقل للأعلى")
                    end); content.addView(up)
                end

                if i < #keys then
                    local down = Button(service); down.setText("🔽"); styleButton(down, "secondary"); down.setPadding(10, 10, 10, 10); down.setOnClickListener(function()
                        keys[i], keys[i+1] = keys[i+1], keys[i]; dashboardOrder = table.concat(keys, ","); refreshUI(); service.asyncSpeak("تم النقل للأسفل")
                    end); local lp = LinearLayout.LayoutParams(-2, -2); lp.leftMargin = 15; content.addView(down, lp)
                end
            end
        end
        refreshUI()

        local saveBtn = Button(service); saveBtn.setText("💾 حفظ الترتيب"); styleButton(saveBtn, "primary")
        saveBtn.setOnClickListener(function() saveSettings(); if settingsDialog then pcall(function() wm.removeView(settingsDialog) end); settingsDialog = nil end end)
        local slp = LinearLayout.LayoutParams(-1, -2); slp.topMargin = 20; settingsDialog.addView(saveBtn, slp)

        pcall(function() wm.addView(settingsDialog, WindowManager.LayoutParams(-1, -1, WindowManager.LayoutParams.TYPE_ACCESSIBILITY_OVERLAY, 0, -3)) end)
    end)
    uiCard.addView(sortBtn, LinearLayout.LayoutParams(-1,-2))
    ...
    local backBtn = Button(service); backBtn.setText("⬅️ رجوع"); styleButton(backBtn, "danger"); backBtn.setOnClickListener(function() hideSettings(); openSettings() end); local lp = LinearLayout.LayoutParams(-1, -2); lp.topMargin = 20; contentL.addView(backBtn, lp)
        ...
        onResults = function(results)
            ...
            if matches and matches.size() > 0 then
                ...
                elseif (lowerRecognizedText == "summarize text" or recognizedText == "لخص النص" or lowerRecognizedText == "résumer le texte") and summarizeEnabled then
                    commandProcessed=true
                    if geminiApiKey == "" then
                        service.asyncSpeak(getFeedbackString("api_key_missing_for_feature", currentDictLangDetails.code, "Google Gemini Summarization"))
                        ...
                        handleCorrection(text, function(correctedText)
                            if newTranslationFeatureEnabled then
                                ...
                                if sourceLangDetails and targetLangDetails then
                                    if geminiApiKey == "" then
                                         service.asyncSpeak(getFeedbackString("api_key_missing_for_feature", currentDictLangDetails.code, "Google Gemini Translation"))
...
local agentStepCount = 0
local MAX_AGENT_STEPS = 5
local agentContextHistory = {}

-- Robust JSON block extractor that handles multiline and markdown cleanly
local function extractJsonBlock(text)
    if not text then return nil end
    local cleaned = text:gsub("^%s*```json%s*", ""):gsub("^%s*```%s*", ""):gsub("%s*```%s*$", "")
    local s_idx = cleaned:find("{")
    if s_idx then
        local e_idx = nil
        for i = #cleaned, s_idx, -1 do
            if cleaned:sub(i, i) == "}" then
                e_idx = i
                break
            end
        end
        if e_idx and e_idx >= s_idx then
            return cleaned:sub(s_idx, e_idx)
        end
    end
    return cleaned
end
...
function showSmartAgentWindow()
    if isAgentProcessing then
        service.asyncSpeak("الوكيل الذكي شغال حالياً، استنى ثواني يا ريس.")
        return
    end
    agentStepCount = 0
    agentContextHistory = {}
...
function startAgentVoiceRecognition()
    ...
    local s, rec = pcall(function() return SpeechRecognizer.createSpeechRecognizer(service) end)
    if not s or not rec then
        service.asyncSpeak("تعذر بدء مسجل الصوت حالياً.")
        isAgentProcessing = false
        return
    end
    smartAgentRecognizer = rec
    ...
    local started = pcall(function() smartAgentRecognizer.startListening(intent) end)
    if not started then
        service.asyncSpeak("حدث خطأ أثناء تشغيل الميكروفون.")
        isAgentProcessing = false
    end
end
...
function processAgentCommand(userCommand)
    ...
    agentStepCount = agentStepCount + 1

    -- Guard against infinite self-healing loops
    if agentStepCount > MAX_AGENT_STEPS then
        service.asyncSpeak("يا ريس، حاولت 5 خطوات وما قدرتش أكمل المهمة بالكامل. وقفت عشان ما أكررش المحاولة بدون فائدة.")
        isAgentProcessing = false
        agentContextHistory = {}
        agentStepCount = 0
        return
    ...
    -- Safety timeout on screenshot capture so agent never freezes indefinitely
    local screenshotHandled = false
    local function onScreenshotDone(imgB64)
        if screenshotHandled then return end
        screenshotHandled = true

        local uiTree = ""
        pcall(function() uiTree = getUiTreeForAgent() end)
...
1. أوامر الكتابة الذكية (Smart Typing):
   - setText("النص"): للكتابة المباشرة في مربع النص (تقوم بالكتابة والتركيز واستخدام الحافظة تلقائياً).
   - paste("النص"): للصق النص في مربع النص المفعل حالياً.
   - للكتابة في حقل محدد: استخدم advancedClick({"نص المربع أو الـ ID"}) ثم setText("النص").
...
- المهمة الطويلة تقسمها لخطوات وخلي status: CONTINUE.]]
        ...
        -- History pruning to prevent prompt overflow
        if #agentContextHistory > 6 then
            local trimmed = {}
            table.insert(trimmed, agentContextHistory[1])
            for i = #agentContextHistory - 3, #agentContextHistory do
                table.insert(trimmed, agentContextHistory[i])
            end
            agentContextHistory = trimmed
        end
        ...
        makeAiRequest(fullPrompt, systemPrompt, imgB64, selectedAgentModelId, function(response)
            if not response or response == "" then
                isAgentProcessing = false
                agentStepCount = 0
                service.asyncSpeak("لم يصل أي رد من الذكاء الاصطناعي.")
                return
            end

            if response:match("^AI Request Failed") or response:match("^Error") then
                isAgentProcessing = false
                agentStepCount = 0
                service.asyncSpeak("فشل في طلب الوكيل: " .. response)
                return
            end

            -- Robust multiline JSON extraction
            local jsonStr = extractJsonBlock(response)
            ...
            if success and data then
                        ...
                        run = function()
                            executeAgentCode(code, function(execSuccess, execMsg)
                                ...
                                if not execSuccess then
                                    status = "CONTINUE"
                                    service.asyncSpeak("الكود واجه مشكلة، هجرب حل بديل.")
                                end
                ...
                agentStepCount = 0
                agentContextHistory = {}
                service.asyncSpeak("معلش يا ريس، حصلت مشكلة في تحليل الرد من الذكاء الاصطناعي.")
            end
        end)
    end

    -- Timeout safety fallback: if screenshot doesn't return within 4.5 seconds, proceed without it
    mainHandler.postDelayed(luajava.createProxy("java.lang.Runnable", {
        run = function()
            if not screenshotHandled then
                onScreenshotDone(nil)
            end
        end
    }), 4500)

    takeScreenshotAndEncode(function(imgB64)
        onScreenshotDone(imgB64)
    end)
end

function executeAgentCode(luaCode, callback)
    local prefixedCode = [[
local service = service or _G.service
local setText = setText or _G.setText or (function(t) return service and service.setText and service.setText(t) end)
local paste = paste or _G.paste or (function(t) return service and service.paste and service.paste(t) end)
local forceClick = forceClick or _G.forceClick
local advancedClick = advancedClick or _G.advancedClick
local smartClick = smartClick or _G.smartClick
local smartStartApp = smartStartApp or _G.smartStartApp
]] .. luaCode
