...
-- Version: 2.2.1 (Critical Bugfix Release)
...
function takeScreenshotAndEncode(callback)
    local function processBitmap(bmp, nodeToRecycle)
        if bmp then
            local success, result = pcall(function()
                ...
                if nodeToRecycle then pcall(nodeToRecycle.recycle, nodeToRecycle) end -- Recycle node after processing
    ...
    if screenshotMode == "focus" then
        ...
        if focusedNode then
            -- Pass the node to the callback so it can be recycled there
...
function main()
    ...
    if _G.assistantState.isFirstRun then
        initTextToSpeech(function(success)
            -- All functions are guaranteed to be defined before this callback is executed.
